def comprar_frutas(morango=0, uva=0):
    ...
    pago pelo cliente.
    '''
    strawberry_price = 2.50 if morango <= 5.00 else 2.20
    grape_price = 1.80 if uva <= 5.00 else 1.50
    
    total_weight = morango + uva
    total_price = (morango * strawberry_price) + (uva * grape_price)
    
    if total_weight > 8.00 or total_price > 25.00:
        total_price *= 0.9
