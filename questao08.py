def media_anual(temperaturas):
    ...
    temps = []
    sum_of_temperatures = sum(temperaturas)
    year_average = sum_of_temperatures/len(temperaturas)
    
    for month, temp in enumerate(temperaturas):
        if temp > year_average:
            temps.append(month)
    return temps
...
def maiores_13(idades,alturas):
    ...
    sum_of_heights = sum(alturas)
    height_average = sum_of_heights/len(alturas)
    heights = []
    
    for index in range(0, len(alturas)):
        if idades[index] > 13 and height_average > alturas[index]:
            heights.append(alturas[index])
            
    return heights

def media_saltos_lista(saltos:list):
...
def lista_de_primos(inicio,fim):
    ...
    isPrime = [False, False]
    primes = []
    for index in range(1, fim):
        isPrime.append(True)
    
    prime = 2
    
    while prime * prime <= fim:
        if isPrime[prime]:
            index = prime * prime
            while index <= fim:
                isPrime[index] = False
    ...
    for prime in range(inicio - 1, fim):
        if isPrime[prime]:
            primes.append(prime)
    return primes
...
def Fibonacci(n):
    ...
    while count < n:
        ...
        new_elm = fibo_list[count - 1] + fibo_list[count - 2]
        fibo_list.append(new_elm)
        count += 1
...
def altera_salarios(salarios):
    ...
    for salary in salarios:
        number_of_sm = salary/724
        if number_of_sm <= 1:
            salary *= 1.2
        elif 1 < number_of_sm <= 2:
            salary *= 1.15
        elif 2 < number_of_sm <= 5:
            salary *= 1.10
        elif number_of_sm > 5:
            salary *= 1.05
        new_salaries.append(round(salary,2))
    return new_salaries
