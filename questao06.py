...
def numeros_sortudos(limite_inferior=1, limite_superior=100000):
    ...
    while limite_inferior < limite_superior + 1:
        number_as_str = str(limite_inferior)
        
        if '2' in number_as_str and '7' not in number_as_str:
            count += 1
        limite_inferior += 1
    return count
...
def ponteironuloville(telefones):
    ...
    def check_adjacent_digit(number_as_str) -> bool:
        for index in range(0,LEN_OF_NUMBER - 1):
            if number_as_str[index] == number_as_str[index + 1]:
                return False
        return True
            
    def check_sum_of_digits(number_as_str) -> bool:
        sum = 0
        for digit in number_as_str:
            sum += int(digit)
        return sum % 2 == 0
    
    def check_first_and_last_digit(number_as_str) -> bool:
        if number_as_str[0] == number_as_str[-1]:
            return False
        return True
    
    def is_valid(number) -> bool:
        return check_adjacent_digit(number) and check_first_and_last_digit(number) and check_sum_of_digits(number)
        
    for number in telefones:
